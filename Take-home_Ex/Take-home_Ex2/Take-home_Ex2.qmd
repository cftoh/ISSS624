...
In this case, the distance between opposite edges should be 375m \* 2. And the length of hexagon should be 375m.

```{r}
BusStop_hexagon_grid = st_make_grid(BusStop, 375, what = "polygons", square = FALSE)
...
Now we see the minimum is 375, which is the the perpendicular distance between the centre of the hexagon and its edges (next hexagon).
...
We manually add in the missing coords and saved it as schools\_.csb
