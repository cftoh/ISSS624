...
-   Creates presentation-ready tables summarizing data sets, regression models etc: **gtsummary**
...
The code chunk below is used to import *MP14_SUBZONE_WEB_PL* shapefile by using `st_read()` of **sf** p
...
To examine if the data file has been imported correctly, and display data structure:
...
```{r}
summary(condo_resale) #display the summary statistics of cond_resale tibble data frame.
```
...
To derive a new variable called *LOG_SELLING_PRICE* by using a log transformation on the variable *SELLING_PRICE*, we use `mutate()` of **dplyr** package.
...
Now, we have the data on *LOG_SELLING_PRICE* , and can plot the *LOG_SELLING_PRICE*:
...
To draw a small multiple histograms (also known as trellis plot) by using `ggarrange()` of [**ggpubr**](https://cran.r-project.org/web/packages/ggpubr/) package.

To create 12 histograms, `ggarrange()` is used to organised these histogram into a 3 columns by 4 rows small multiple plot.
...
To reveal the geospatial distribution condominium resale prices in Singapore. The map will be prepared by using **tmap** package.
...
Building hedonic pricing models for condominium resale units using [`lm()`](https://www.rdocumentation.org/packages/stats/versions/3.5.2/topics/lm) of R base.
...
Refer to [Tutorial: tbl_regression](https://www.danieldsjoberg.com/gtsummary/articles/tbl_regression.html) for more customisation options
...
[**olsrr**](https://olsrr.rsquaredacademy.com/). It provides a collection of very useful methods for building better multiple linear regression models:
...
If want to do formal statistical test methods, the [`ols_test_normality()`](https://olsrr.rsquaredacademy.com/reference/ols_test_normality.html) of **olsrr** package can be used as shown :
...
To calibrate the gwr-based hedonic pricing model by using adaptive bandwidth approach.
